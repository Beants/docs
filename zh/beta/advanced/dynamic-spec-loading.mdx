...
### 为什么 Codex 会把第一次 patch 拦下一次

Codex 不要求修改文件前必须先读文件。第一次 patch 匹配到 spec 时，hook 会返回
完整规则，并拒绝这一轮 patch：
...
内的 spec 不会产生输出。这是一次上下文交付握手，不是策略拒绝。
...
| `tools`                  | 可以触发匹配的 Claude Code 工具名。Codex 的 `apply_patch` 映射为逻辑上的 `Edit`。空列表表示禁用全部触发。 |
...
使用独立的交付历史。`SessionStart(source=clear|compact)` 会为当前 session
记录共享 reset 标记，因此被压缩掉的规则会在下一次匹配时重新交付。

Hook 不会解析 Claude Code 或 Codex 的 transcript 内容。Transcript 格式属于
宿主内部实现，不是 Trellis 的契约。

匹配和状态故障采用 fail-open：frontmatter 格式错误、路径缺失、状态不可写或
hook 内部异常，都不能破坏宿主工具调用。唯一有意阻断的情况，是 Codex 刚刚
收到完整约束 spec 时的第一次 patch。
