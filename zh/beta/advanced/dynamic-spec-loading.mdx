...
| Pi Agent    | `write` / `edit` 的 `tool_call`；`read` 的 `tool_result` | 新规则首次命中时阻断一次；Read 上下文和不阻断的 ticket 会追加到对应工具结果。              |
| 其他平台    | Pull mode                                                | 通过 `get_context.py` 主动查询匹配的 spec 路径。                                           |

### 为什么 Codex、OpenCode 和 Pi 会把第一次修改拦下一次

Codex、OpenCode 和 Pi 都不要求修改文件前必须先读文件。第一次修改匹配到 spec
时，Trellis 会返回完整规则，并阻断这一轮调用：
...
没有直接返回 `additionalContext` 的字段，因此上下文会放进工具错误里。Pi 会把
spec 作为阻断原因返回，Pi 随后会把它保存为模型可见的错误工具结果。这是一次上下文
交付握手，不是策略拒绝。
...
| `tools`                  | 可以触发匹配的逻辑工具名。Codex 和 OpenCode 的 `apply_patch` 映射为 `Edit`；OpenCode 和 Pi 的 `write` / `edit` 映射为 `Write` / `Edit`。空列表表示禁用全部触发。 |
...
`session.compacted` 映射为相同的 compact reset；Pi 映射 `session_compact`。
因此，被压缩掉的规则会在下一次匹配时重新交付。

Provider 不会解析 Claude Code、Codex、OpenCode 或 Pi 的 transcript 内容。
Transcript 格式属于宿主内部实现，不是 Trellis 的契约。

匹配和状态故障采用 fail-open：frontmatter 格式错误、路径缺失、状态不可读或
不可写、hook 内部异常，都不能破坏宿主工具调用。唯一有意阻断的情况，是 Codex
、OpenCode 或 Pi 刚刚收到完整约束 spec、且交付状态已经成功记录时的第一次修改。
