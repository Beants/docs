...
npm install -g @mindfoldhq/trellis@latest    # CLI 0.5.x 或更早（还没有 upgrade 命令）
```
...
如果你的 CLI 早于 0.6.0，`trellis upgrade` 命令还不存在——第 1 步改用 `npm install -g @mindfoldhq/trellis@latest`，之后的升级就能用 `trellis upgrade` 了。
