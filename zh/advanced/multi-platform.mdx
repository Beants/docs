...
还要走一次 `/hooks` 审批；Kiro 的 Agent Hook 需用户自行 wiring） · ❌ 平台不暴露这个事件 ·
...
Codex 0.129+ 还要在 TUI 里跑一次 `/hooks` 命令，审批 Trellis 安装的 `UserPromptSubmit` hook，否则 hook 不会激活、workflow 指引不会自动注入。不开启时 Codex 走纯 prelude（每次读 `AGENTS.md`），内容一致，只是没有事件驱动注入。
