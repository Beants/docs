...
Skill 是 Trellis 的主要扩展点。原生带有的大部分工作流（brainstorm、before-dev、check、update-spec、break-loop）都以 auto-trigger skill 的形式交付。本章说明如何添加自己的 skill。
...
Skill 在全部 21 个配置平台上都可用，位置按平台不同：
...
| Codex       | `.agents/skills/{name}/SKILL.md`（共享层；`.codex/skills/` 会建成空目录，供你放自己的 skill） |
| Kiro        | `.kiro/skills/{name}/SKILL.md`                                                                |
| Gemini CLI  | `.agents/skills/{name}/SKILL.md`（Gemini CLI 0.40+ 直接读共享层）                             |
| Qoder       | `.qoder/skills/{name}/SKILL.md`                                                               |
| CodeBuddy   | `.codebuddy/skills/{name}/SKILL.md`                                                           |
| Copilot     | `.github/skills/{name}/SKILL.md`                                                              |
| Droid       | `.factory/skills/{name}/SKILL.md`                                                             |
| Pi Agent    | `.agents/skills/{name}/SKILL.md`（Pi 读共享层）                                               |
...
`.agents/skills/{name}/SKILL.md`（[agentskills.io](https://agentskills.io) 跨平台共享层）可被 Amp、Cline、Deep Agents、Firebender、Warp 等读该标准的 agent 直接消费。
