...
Supported platforms: Claude Code, Cursor, OpenCode, Codex, Kiro, Kilo, Gemini CLI, Antigravity, Devin, Qoder, CodeBuddy, GitHub Copilot, Droid, Pi Agent, Oh My Pi, Reasonix, ZCode, Trae, Grok Build, Kimi Code, Snow CLI, plus any agent that reads the `.agents/skills/` standard (Amp, Cline, Deep Agents, Firebender, Warp, and more).
...
| Platform support      | Cursor only                       | Claude Code only                 | Per platform              | **21 configured platforms + shared skill ecosystem**                                          |
