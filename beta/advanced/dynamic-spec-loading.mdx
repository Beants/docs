...
| Pi Agent        | `tool_call` for `write` / `edit`; `tool_result` for `read` | Blocks a newly governed mutation once. Read context and non-blocking tickets are appended to the matching tool result.                      |
| Other platforms | Pull mode                                                  | Query matching spec paths explicitly with `get_context.py`.                                                                                 |

### Why Codex, OpenCode, and Pi block the first mutation once

Codex, OpenCode, and Pi do not require a file read before an edit. When a
mutation first matches a spec, Trellis returns the full rules and blocks that
call once:
...
`additionalContext` return field. Pi returns the context as the reason for a
blocked tool call; Pi records that reason as a model-visible error tool result.
This is a context-delivery handshake, not a policy rejection.
...
| `tools`                  | Logical tool names that can trigger matching. Codex and OpenCode `apply_patch` use `Edit`; OpenCode and Pi `write` / `edit` use `Write` / `Edit`. An empty list disables every trigger. |
...
reset; Pi maps `session_compact`. Rules removed by compaction are therefore
delivered again.

The provider does not parse Claude Code, Codex, OpenCode, or Pi transcript
contents. Transcript formats are host internals and are not part of this
contract.
...
or Pi mutation that has just received a full governing spec with persisted
delivery state.
